...
print("Starting all-mode scanning (version check)...")
found = False
    ...
    for chunk in chunks:
        if len(chunk) >= 32:
            for j in range(len(chunk) - 31):
                ...
                # Check version byte helper
                # Bytes 44..47 in decrypted stream must be \x01\x00\x00\x00 or \x02\x00\x00\x00
                
                # 1. CFB, IV at start (offset 16)
                try:
                    cipher = AES.new(candidate, AES.MODE_CFB, iv=pck_data[:16], segment_size=128)
                    dec = cipher.decrypt(pck_data[16:96])
                    if dec[45] == 0 and dec[46] == 0 and dec[47] == 0 and dec[44] in (1, 2):
                        print(f"\nFOUND KEY (CFB, IV-start)! Key: {candidate.hex()}")
                        print(f"Magic: {dec[40:44]}")
                        found = True
                        break
                except:
                    pass
                
                # 2. CFB, Zero-IV (offset 0)
                try:
                    zero_iv = b"\x00" * 16
